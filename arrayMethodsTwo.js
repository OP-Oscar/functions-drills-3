...
const citiesI = cities.filter(item => item[item.length-1] === 'i')
console.log(citiesI)
...
fruits.forEach(el =>{
  ...
  console.log(`The fruit with index ${fruits.indexOf(el)}  is neither an apple or orange`)
}
})
...
console.log(foods.filter(item => item.tags.includes('rice')).reduce((acc, curr)=>acc + curr.price, 0))

// THE TOTAL
// Total price of foods that have "rice" is 36
