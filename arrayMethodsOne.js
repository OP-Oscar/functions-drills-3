...
// const evenNumbers = mixedNumbers.filter(function even(item){ return item % 2 === 0 }) //function form
const evenNumbers = mixedNumbers.filter(element => element %2 === 0) //arrow form
...
// const postTaxPrices = prices.map(function taxes(ele){ return (ele*1.07).toFixed(2)}) //=> function form, also added toFixed
const postTaxPrices  = prices.map(ele => (ele*1.07).toFixed(2) ); // => arrow form
...
// const totalPopulation = populations.reduce(function (el,dos){return el+dos}) // => function form
const totalPopulation = populations.reduce((acc, curr) => acc + curr ) // => arrow method
...
// const myStrongest = monstersInYourPocket.filter(function highHealth(ele){return ele.CP > 200}) // => function style
const myStrongest = monstersInYourPocket.filter(element => element.CP > 200) // => arrow method
...
// const ordersTotals = orders.map(function taxes(element){return (element.price * (element.tax + 1)).toFixed(2)}) // => function method
const ordersTotals = orders.map(element => (element.price * (element.tax +1)).toFixed(2)) // => arrow style
...
const bobsOrders = purchases.filter(el=> el.owner === "Bob").reduce((acc, curr) => acc + curr.price, 0)
